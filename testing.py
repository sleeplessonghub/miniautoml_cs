        ...
        if target != '-':
          if train[target].dtypes == float or train[target].dtypes == int:
            ...
            if train[target].nunique() <= 5:
              ...
              st.dataframe(train[target].value_counts(sort = True).rename('Categorical Frequency').reset_index().astype(str), height = 'auto', column_config = {target: st.column_config.Column(width = 325), 'Categorical Frequency': st.column_config.Column(width = 200)}, hide_index = True)
            elif train[target].nunique() > 5:
              st.write('Class 1 Label Selection (Scrollable):')
              st.dataframe(train[target].value_counts(sort = True).rename('Categorical Frequency').reset_index().astype(str), height = 213, column_config = {target: st.column_config.Column(width = 325), 'Categorical Frequency': st.column_config.Column(width = 200)}, hide_index = True)
