      ...
      if submitted_2_ref == True:
        if unassigned_count_2 > 0:
          ...
          if is_object == False: # Regression modeling
            ...
            lgbm_reg.fit(feature_train, target_train, callbacks = [lgbm.early_stopping(stopping_rounds = 5)])
            ...
            lgbm_class.fit(feature_train, target_train, callbacks = [lgbm.early_stopping(stopping_rounds = 5)])
            ...
            lgbm_class_rs.fit(feature_train_balanced, target_train_balanced, callbacks = [lgbm.early_stopping(stopping_rounds = 5)])
